...
impl JmsComponent {
    ...
    pub fn heartbeat_ok(&self) -> bool {
        (chrono::Local::now() - self.last_tick)
            < chrono::Duration::milliseconds(self.timeout_ms as i64)
    }

    pub fn heartbeat_ok_for(id: &str, kv: &kv::KVConnection) -> anyhow::Result<bool> {
        Ok(
            (chrono::Local::now() - Self::get_last_tick(id.to_owned(), kv)?)
                < chrono::Duration::milliseconds(Self::get_timeout_ms(id.to_owned(), kv)? as i64),
        )
    }
}
