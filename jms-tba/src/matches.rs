...
use jms_core_lib::{
    db::{Singleton, Table},
    models::{self, MatchType, PlayoffModeType},
    scoring::scores::{EndgameType, GamepieceType, Link, MatchScoreSnapshot, SnapshotScore},
};
use log::error;
        ...
        for m in matches {
            if m.match_type != MatchType::Test {
                let latest_score = scores
                    .get(&m.id)
                    .and_then(|cms| cms.scores.last())
                    .map(|x| Into::<MatchScoreSnapshot>::into(x.clone()));

                // Try to convert our format into TBA's format
                let (comp_level, set, match_n) = match (
                    &playoff_type.mode,
                    m.match_type,
                    m.round,
                    m.set_number,
                    ...
                    (PlayoffModeType::Bracket, MatchType::Playoff, r, s, m) => match (r, s) {
                ...
                match (comp_level, set, match_n) {
                    (Some(comp_level), set_number, match_number)
                        if set_number != 0 && match_number != 0 =>
                    {
                        let red_teams = m
                            .red_teams
                            .iter()
                            .filter_map(|x| x.map(|t| TBATeam::from(t)))
                            .collect();
                        let blue_teams = m
                            .blue_teams
                            .iter()
                        ...
                        let tba_match = TBAMatch {
                            ...
                            alliances: TBAMatchAlliances {
                                red: TBAMatchAlliance {
                                    ...
                                    score: latest_score
                                        .as_ref()
                                        .map(|x| x.red.derived.total_score as isize),
                                },
                                blue: TBAMatchAlliance {
                                    teams: blue_teams,
                                    score: latest_score
                                        .as_ref()
                                        .map(|x| x.blue.derived.total_score as isize),
                                },
                            },
                            time_str: Some(m.start_time.format("%l:%M %p").to_string()),
                            time_utc: Some(
                                chrono::DateTime::<chrono::Utc>::from(m.start_time)
                                    .format("%+")
                                    .to_string(),
                            ),
                        };
...
impl TBA2023ScoreBreakdown {
    ...
    pub fn convert_community(
        community: Vec<Vec<GamepieceType>>,
...
impl From<SnapshotScore> for TBA2023ScoreBreakdown {
    fn from(score: SnapshotScore) -> Self {
        ...
        Self {
            ...
            autoBridgeState: if live.charge_station_level.auto {
                "Level"
            } else {
                "NotLevel"
            },
            autoGamePieceCount: live
                .community
                .auto
                .iter()
                .flat_map(|x| x)
                .filter(|gp| *gp != &GamepieceType::None)
                .count() as isize,
            ...
            endGameBridgeState: if live.charge_station_level.teleop {
                "Level"
            } else {
                "NotLevel"
            },
