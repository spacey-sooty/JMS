...
use jms_core_lib::{
    db::{Singleton, Table},
    models::{
        Alliance, CommittedMatchScores, Match, MatchType, MaybeToken, Permission, TeamRanking,
    },
    schedule::generators::MatchGeneratorRPCClient,
    scoring::scores::{LiveScore, MatchScore, MatchScoreSnapshot, ScoreUpdateData},
};
use uuid::Uuid;
...
pub trait ScoringWebsocket {
    ...
    async fn do_score_update(
        kv: &kv::KVConnection,
        update: ScoreUpdateData,
    ) -> anyhow::Result<MatchScoreSnapshot> {
    ...
    async fn score_update(
        ...
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::Scoring])?;
    ...
    async fn latest_scores(
        &self,
        ctx: &WebsocketContext,
    ) -> anyhow::Result<Option<CommittedMatchScores>> {
        let mut scores = CommittedMatchScores::all(&ctx.kv)?
            .into_iter()
            .filter(|s| s.scores.len() > 0)
            .collect::<Vec<_>>();
    ...
    #[endpoint]
    async fn get_matches_with_scores(
        &self,
        ctx: &WebsocketContext,
        _token: &MaybeToken,
    ...
    async fn get_committed(
        ...
        _token: &MaybeToken,
        match_id: String,
    ) -> anyhow::Result<CommittedMatchScores> {
        CommittedMatchScores::get(&match_id, &ctx.kv)
    }

    #[endpoint]
    async fn new_committed_record(
        &self,
        ctx: &WebsocketContext,
        token: &MaybeToken,
        match_id: String,
    ...
    async fn push_committed_score(
        ...
        MatchGeneratorRPCClient::update_playoffs(&ctx.mq)
            .await?
        ...
        for m in Match::all(&ctx.kv)? {
            ...
            if m.match_type == ty && m.ready {
                match CommittedMatchScores::get(&match_id, &ctx.kv) {
                    ...
                    Err(_) => {
                        let mut c = CommittedMatchScores {
                            match_id: match_id,
                            scores: vec![],
                            last_update: chrono::Local::now(),
                        };
