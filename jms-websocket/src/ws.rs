...
use jms_core_lib::models::{MaybeToken, UserToken};
...
impl Websocket {
    ...
    pub async fn run(
        &mut self,
        stream: TcpStream,
        mut global_bcast: broadcast::Receiver<(String, serde_json::Value)>,
        timeout: Duration,
    ) -> anyhow::Result<()> {
...
impl Websockets {
    ...
    pub async fn register<T: 'static + WebsocketHandler + Send + Sync>(
        &mut self,
        loop_time: Duration,
        name: &str,
        handler: T,
        ...
        self.handlers
            .insert(name.to_owned(), (loop_time, Box::new(handler)));
    }
    ...
    pub fn to_schema(&self) -> RootSchema {
        ...
        generator.definitions_mut().insert(
            "WebsocketPublish".to_owned(),
            Schema::Object(publish_schema),
        );
        ...
        generator.definitions_mut().insert(
            "WebsocketRpcRequest".to_owned(),
            Schema::Object(rpc_request_schema),
        );
        ...
        generator.definitions_mut().insert(
            "WebsocketRpcResponse".to_owned(),
            Schema::Object(rpc_response_schema),
        );
    ...
    pub async fn begin(self, ctx: WebsocketContext) -> anyhow::Result<()> {
        ...
        let mut handler_ints: Vec<(String, Interval)> = handlers
            .iter()
            .map(|(k, v)| (k.clone(), interval(v.0)))
            .collect();
