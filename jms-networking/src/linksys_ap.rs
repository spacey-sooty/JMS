...
use crate::{
    ssh::{CommandResult, SSHSession},
    NetworkConfig,
};

pub async fn configure_ap_teams(
    ...
    let session =
        SSHSession::connect(addr, &settings.radio_username, &settings.radio_password).await?;

    let mut cfgs = vec![];

    for (i, (team, wpa_key)) in vec![
        &config.blue1,
        &config.blue2,
        &config.blue3,
        &config.red1,
    ...
    .enumerate()
    {
        ...
        match wpa_key {
            Some(wpa_key) => {
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].disabled='0'",
                    iface_num
                ));
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].ssid='{}'",
                    iface_num, team
                ));
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].key='{}'",
                    iface_num, wpa_key
                ));
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].encryption='psk2'",
                    iface_num
                ));
            }
            None => {
                ...
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].disabled='1'",
                    iface_num
                ));
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].ssid='{}'",
                ...
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].key='unoccupied'",
                    iface_num
                ));
                cfgs.push(format!(
                    "set wireless.@wifi-iface[{}].encryption='psk2'",
...
async fn do_uci(session: &SSHSession, admin: bool, cmds: &[&str]) -> anyhow::Result<CommandResult> {
    ...
    if admin {
        let reply = session
            .run("/sbin/wifi down radio1; /sbin/wifi up radio1")
            .await?;
        if !reply.success() {
        ...
        let reply = session
            .run("/sbin/wifi down radio0; /sbin/wifi up radio0")
            .await?;
        if !reply.success() {
