    ...
    pub fn has(&self, required: &Permission) -> bool {
        match (self, required) {
            ...
            (
                ...
                | Permission::ManagePlayoffs
                | Permission::ManageAwards
                | Permission::ManageAlliances
                | Permission::MatchFlow
                | Permission::Estop
                | Permission::ManageAudience
...
impl User {
    ...
    pub fn get(username: &str, kv: &KVConnection) -> anyhow::Result<Self> {
        ...
        let key = keys
            .iter()
            .find(|x| x.to_lowercase() == username.to_lowercase())
...
impl MaybeToken {
    pub fn auth(&self, kv: &KVConnection) -> anyhow::Result<User> {
        match &self.0 {
            Some(token) => match User::get(&token.user, kv) {
                Ok(user) => {
                ...
                Err(_) => anyhow::bail!(
                    "Token is for a user who no longer exists. Please refresh the page."
                ),
            },
