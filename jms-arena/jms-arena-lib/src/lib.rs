...
use jms_base::{
    kv,
    mq::{MessageQueueChannel, MessageQueueSubscriber},
};
use jms_core_lib::{
    db::{DBDuration, Table},
    models::{Alliance, AllianceParseError, AllianceStationId, JmsComponent},
};
...
/* ALLIANCE STATIONS */
#[derive(
    jms_macros::DbPartialUpdate,
    jms_macros::Updateable,
    Debug,
    Clone,
    PartialEq,
    Eq,
...
impl ArenaStateHook {
    ...
    pub async fn failure<E: ToString>(
        &self,
        err: E,
        mq: &MessageQueueChannel,
    ) -> anyhow::Result<()> {
