use jms_core_lib::{
    db::{Singleton, Table},
    models::{MaybeToken, Permission, PlayoffAlliance, PlayoffMode},
};
...
pub trait AlliancesWebsocket {
    ...
    async fn create(
        ...
        token: &MaybeToken,
    ) -> anyhow::Result<Vec<PlayoffAlliance>> {
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::ManageAlliances])?;
