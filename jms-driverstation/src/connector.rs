...
use jms_arena_lib::{
    AllianceStation, ArenaState, MatchPlayState, SerialisedLoadedMatch, ARENA_MATCH_KEY,
    ARENA_STATE_KEY,
};
use jms_base::kv::KVConnection;
use jms_core_lib::{
    db::Table,
    models::{Alliance, AllianceStationId},
};
use jms_driverstation_lib::{DriverStationReport, RobotState, TournamentLevel};
use log::error;
use tokio::{
...
use crate::{
    tcp_codec::{DSTCPCodec, Ds2FmsTCPTags, Fms2DsStationStatus, Fms2DsTCP, Fms2DsTCPTags},
    udp_codec::{DSUDPCodec, Ds2FmsUDP, Ds2FmsUDPTags, Fms2DsUDP},
};
    ...
    async fn _encode_udp_update(&self, _team: usize) -> Option<Fms2DsUDP> {
        if let Some(station) = self._get_desired_alliance_station().await {
            if let Ok(arena_state) = self.kv.json_get::<ArenaState>(ARENA_STATE_KEY, "$") {
                let (mut command_enable, command_state, remaining) =
                    match self
                        .kv
                        .json_get::<SerialisedLoadedMatch>(ARENA_MATCH_KEY, "$")
                    {
                        Ok(m) => match m.state {
        ...
        match desired {
            ...
            Some(stn_desired) => {
                match actual {
                    ...
                    Some(stn_actual) if stn_actual.id == stn_desired.id => {
                        Fms2DsStationStatus::Good
                    }
