...
use jms_core_lib::{
    db::Table,
    models::{self, MatchType},
    schedule::generators::QualsMatchGeneratorParams,
};
use log::info;
...
impl QualsMatchGenerator {
    ...
    pub fn generate(
        ...
        let teams: Vec<usize> = models::Team::all(kv)?
            .iter()
            .filter(|&t| t.schedule)
            .map(|t| t.number)
            .collect();
            ...
            match block.block_type {
                models::ScheduleBlockType::Qualification { cycle_time } => {
                    ...
                    while (offset + cycle_time.0) <= block.end_time {
                        if let Some(m) = existing_matches.iter().find(|m| {
                            m.start_time >= offset && m.start_time <= (offset + cycle_time.0)
                        }) {
        ...
        let team_sched = generation_result.schedule.contextualise(&teams);

        let gen_end_t = chrono::Local::now();
        info!("Match Generation Completed in {}", gen_end_t - gen_start_t);

        // Commit
        let match_n_offset = existing_matches
            .iter()
            .filter(|x| x.match_type == MatchType::Qualification)
            .map(|x| x.match_number)
            .max()
            .unwrap_or(0);
