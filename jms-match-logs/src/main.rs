...
async fn logs_svc(kv: kv::KVConnection) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if let Some(current_match) = current_match {
            let match_time = current_match
                .match_time
                .map(|mt| mt.0.num_milliseconds())
                .unwrap_or(0);

            if current_match.state == MatchPlayState::Auto
                || current_match.state == MatchPlayState::Pause
                || current_match.state == MatchPlayState::Teleop
            {
                ...
                for report in reports {
                    ...
                    let entry = match logs.entry(report.team as usize) {
                        ...
                        Entry::Vacant(v) => v.insert(MatchLog {
                            match_id: current_match.match_id.clone(),
                            team: report.team as usize,
                            timeseries: vec![],
                        }),
                    };
            ...
            if current_match.state == MatchPlayState::Complete
                && last_state != Some(MatchPlayState::Complete)
            {
                // Commit records
