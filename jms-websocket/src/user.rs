use jms_core_lib::{
    db::Table,
    models::{MaybeToken, Permission, User, UserToken, UserUpdate},
};
...
pub trait UserWebsocket {
    ...
    async fn auth_with_token(
        ...
        if User::ids(&ctx.kv)?.is_empty() {
            ...
            Ok(AuthResult::AuthSuccessNewPin {
                user,
                token: utoken,
            })
    ...
    async fn auth_with_pin(
        ...
        let mut user = User::get(&username, &ctx.kv)
            .map_err(|_e| anyhow::anyhow!("No User with that username"))?;
    ...
    async fn logout(&self, ctx: &WebsocketContext, token: &MaybeToken) -> anyhow::Result<()> {
        ...
        let index = user
            .tokens
            .iter()
            .position(|x| x == &token.0.as_ref().unwrap().token)
            .unwrap();
            ...
            if tok_user.id() == user.id() {
                match &update {
                    UserUpdate::permissions(perms) => {
                        if tok_user.permissions.contains(&Permission::Admin)
