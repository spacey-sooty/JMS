...
impl ScoringService {
    pub async fn run(self) -> anyhow::Result<()> {
        let mut publish_sub: mq::MessageQueueSubscriber<String> = self
            .mq
            .subscribe(
                "arena.scores.publish",
                "core-scoring-publish",
                "ScoringService",
                false,
            )
            .await?;
        let mut ranking_update_interval =
            tokio::time::interval(Duration::from_millis(10 * 60 * 1000)); // 10 mins, just in case it doesn't get triggered elsewhere
        loop {
            tokio::select! {
