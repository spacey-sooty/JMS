...
use jms_core_lib::{
    db::{Singleton, Table},
    models::{
        Award, AwardRecipient, CommittedMatchScores, Match, MatchType, PlayoffAlliance,
        PlayoffMode, PlayoffModeType, ScheduleBlock, ScheduleBlockType, Team,
    },
};
use log::{info, warn};
...
pub enum GenerationUpdate {
    ...
    TournamentWon { winner: usize, finalist: usize },
}
...
impl PlayoffMatchGenerator {
    pub fn reset(kv: &kv::KVConnection) -> anyhow::Result<()> {
        ...
        if matches.iter().any(|x| {
            x.played && (x.match_type == MatchType::Playoff || x.match_type == MatchType::Final)
        }) {
            anyhow::bail!("Can't reset the Playoffs Schedule if it's already in motion!");
        }

        let to_delete = matches
            .iter()
    ...
    pub fn update(kv: &kv::KVConnection) -> anyhow::Result<()> {
        ...
        let blocks = ScheduleBlock::sorted(kv)?
            .into_iter()
            .filter(|x| x.block_type == ScheduleBlockType::Playoff);

        let update = match playoff_mode.mode {
            PlayoffModeType::Bracket | PlayoffModeType::DoubleBracket => {
                bracket_update(&playoff_mode, &matches, &scores)?
            }
        };
        ...
        match update {
            GenerationUpdate::MatchUpdates(updates) => {
                ...
                for item in updates {
                    hydrated.push(HydratedPlayoffScheduleItem {
                        duration: None,
                        item,
                    });
                }
                ...
                for item in hydrated.iter_mut() {
                    match item.item {
                        PlayoffScheduleItem::AwardsBreak => {
                            item.duration = Some(playoff_mode.minimum_round_break.0.clone())
                        }
                        _ => (),
                    }
                }
                ...
                let net_time = total_time
                    - hydrated
                        .iter()
                        .filter_map(|x| x.duration)
                        .fold(Duration::zero(), |a, b| a + b);
                let mut cycle_time = net_time
                    / (hydrated
                        .iter()
                        .filter(|x| matches!(x.item, PlayoffScheduleItem::Match(..)))
                        .count() as i32);
                    ...
                    match item.item {
                        PlayoffScheduleItem::Match(m) => {
                            let (red_teams, red_alliance) = match &m.red {
                                PlayoffAllianceDescriptor::Alliance(a) => (
                                    alliances.get(a).map(|x| x.teams.clone()).unwrap_or(vec![]),
                                    Some(*a),
                                ),
                                _ => (vec![], None),
                            };
                            ...
                            if let Ok(mut existing) = Match::get(&id, kv) {
                                if !existing.played {
                                    ...
                                    existing.red_teams = vec![
                                        red_teams.get(0).copied(),
                                        red_teams.get(1).copied(),
                                        red_teams.get(2).copied(),
                                    ];
                                    existing.blue_teams = vec![
                                        blue_teams.get(0).copied(),
                                        blue_teams.get(1).copied(),
                                        blue_teams.get(2).copied(),
                                    ];
                                    existing.ready =
                                        red_alliance.is_some() && blue_alliance.is_some();
                                    existing.insert(kv)?;
                                }
                            } else {
            ...
            GenerationUpdate::TournamentWon { winner, finalist } => {
                ...
                if let Some(winning_alliance) = alliances.get(&winner) {
                    let recipients: Vec<AwardRecipient> = winning_alliance
                        .teams
                        .iter()
                        .map(|t| AwardRecipient {
                            team: Some(
                                teams
                                    .get(t)
                ...
                if let Some(finalist_alliance) = alliances.get(&finalist) {
                    let recipients: Vec<AwardRecipient> = finalist_alliance
                        .teams
                        .iter()
                        .map(|t| AwardRecipient {
                            team: Some(
