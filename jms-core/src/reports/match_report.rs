...
use super::{pdf_table, render_header, report_pdf};

fn render_team(
    team: Option<&Option<usize>>,
    highlight: Option<usize>,
    alliance: models::Alliance,
    teams: &HashMap<usize, Team>,
) -> style::StyledString {
    let team = team.and_then(|t| t.clone());
    let t_string = team.map_or("".to_owned(), |t| {
        teams
            .get(&t)
            .map(|team| team.display_number.clone())
            .unwrap_or(format!("{}", t))
    });
...
pub fn match_report(
    mtype: models::MatchType,
    kv: &kv::KVConnection,
) -> Result<ReportData, Box<dyn std::error::Error>> {
    let mut buf = vec![];

    let event_details = models::EventDetails::get(kv)?;
    let event_name = event_details
        .event_name
        .unwrap_or("Unnamed Event".to_owned());
    let matches = models::Match::sorted(kv)?
        .into_iter()
        .filter(|m| m.match_type == mtype)
        .collect();
