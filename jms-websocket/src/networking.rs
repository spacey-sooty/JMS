...
use jms_networking_lib::{JMSNetworkingRPCClient, NetworkingSettings, NetworkingSettingsUpdate};
...
pub trait NetworkingWebsocket {
    ...
    async fn settings(
        ...
        token: &MaybeToken,
    ) -> anyhow::Result<NetworkingSettings> {
        token
            .auth(&ctx.kv)?
    ...
    async fn reload_admin(&self, ctx: &WebsocketContext, token: &MaybeToken) -> anyhow::Result<()> {
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::FTA])?;
        JMSNetworkingRPCClient::configure_admin(&ctx.mq)
            .await?
            .map_err(|e| anyhow::anyhow!(e))
    }

    #[endpoint]
    async fn force_reprovision(
        &self,
        ctx: &WebsocketContext,
        token: &MaybeToken,
    ) -> anyhow::Result<()> {
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::FTA])?;
        JMSNetworkingRPCClient::force_reprovision(&ctx.mq)
            .await?
            .map_err(|e| anyhow::anyhow!(e))
    }
}
