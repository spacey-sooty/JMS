...
impl ScheduleGenerator {
    ...
    pub fn cooccurrence_matrix(&self, schedule: &Schedule) -> Option<na::DMatrix<usize>> {
        let mut cooccurrence: na::DMatrix<usize> =
            na::DMatrix::zeros(self.num_teams, self.num_teams);
    ...
    pub fn station_matrix(&self, schedule: &Schedule) -> na::DMatrix<usize> {
        let mut stations: na::DMatrix<usize> =
            na::DMatrix::zeros(self.num_stations, self.num_teams);
    ...
    pub fn generate(
        &self,
        anneal_team_balance: Annealer,
        anneal_station_balance: Annealer,
    ) -> GenerationResult {
        ...
        info!(
            "Seed schedule generated (in {:.2}s)",
            (t1 - t0).as_secs_f32()
        );
        debug!("{}", seed.0);
