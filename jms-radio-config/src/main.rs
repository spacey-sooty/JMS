...
async fn main() -> anyhow::Result<()> {
    ...
    let iface = args
        .iface
        .clone()
        .and_then(|i| valid_ifaces.iter().find(|iface| iface.name == i));
    let iface = iface.cloned().unwrap_or_else(|| {
        inquire::Select::<LinkMetadata>::new("Select Interface", valid_ifaces)
            .prompt()
            .unwrap()
    });
...
async fn field_mode(args: &Args, iface: LinkMetadata) -> anyhow::Result<()> {
    ...
    match args.team {
        ...
        None => {
            interactive::run_interactive(iface, interactive::InteractiveProps::Field(all_keys))?
        }
    }
...
async fn home_mode(args: &Args, iface: LinkMetadata) -> anyhow::Result<()> {
    match args.team {
        ...
        None => interactive::run_interactive(
            iface,
            interactive::InteractiveProps::Home(args.key.clone()),
        )?,
    };
    Ok(())
}
                ...
                EditView::new()
                    .on_edit(|s, number, _| {
                        ...
                        let t = {
                            ...
                            match &dat.props {
                                InteractiveProps::Field(keys) => {
                                    (dat.team, keys.contains_key(&dat.team))
                                }
        ...
        siv.add_layer(
            Dialog::new()
                ...
                .button("Image My Radio!", move |s| {
                    ...
                    let dat = { s.user_data::<Data>().unwrap().clone() };
                    ...
                    let key = match &dat.props {
                        ...
                        InteractiveProps::Home(_) | InteractiveProps::Basic => {
                            Some(dat.home_key.clone())
                        }
                    };
                            ...
                            cb.send(Box::new(move |s| {
                                ...
                                match result {
                                    Ok(()) => {
                                        ...
                                        tv.set_style(ColorStyle::front(Color::Light(
                                            BaseColor::Green,
                                        )));
                                    }
                                    Err(err) => {
                                        tv.set_content(format!(
                                            "Imaging Failed. Try again. {}",
                                            err
                                        ));
                                        tv.set_style(ColorStyle::front(Color::Light(
                                            BaseColor::Red,
