...
use jms_arena_lib::{
    AllianceStation, AllianceStationUpdate, ArenaRPCClient, ArenaSignal, ArenaState,
    SerialisedLoadedMatch, ARENA_MATCH_KEY, ARENA_STATE_KEY,
};
use jms_core_lib::{
    db::Table,
    models::{AllianceStationId, Match, MatchType, MaybeToken, Permission},
};
...
pub trait ArenaWebsocket {
    ...
    async fn current_match(
        &self,
        ctx: &WebsocketContext,
    ) -> anyhow::Result<Option<SerialisedLoadedMatch>> {
    ...
    async fn load_match(
        ...
        token.auth(&ctx.kv)?.require_permission(&[
            Permission::FTA,
            Permission::FTAA,
            Permission::Scorekeeper,
        ])?;
        ArenaRPCClient::load_match(&ctx.mq, match_id)
            .await
            .map_err(|e| anyhow::anyhow!(e))?
            .map_err(|e| anyhow::anyhow!(e))
    }
    ...
    async fn load_test_match(
        ...
        let max_test_match = Match::all(&ctx.kv)?
            .iter()
            .filter(|m| m.match_type == MatchType::Test)
            .map(|x| x.set_number)
            .max()
            .unwrap_or(0);
        let m = Match {
        ...
        m.insert(&ctx.kv)?;
        ArenaRPCClient::load_match(&ctx.mq, m.id.clone())
            .await
            .map_err(|e| anyhow::anyhow!(e))?
            .map_err(|e| anyhow::anyhow!(e))
    }
    ...
    async fn unload_match(&self, ctx: &WebsocketContext, token: &MaybeToken) -> anyhow::Result<()> {
        ...
        ArenaRPCClient::unload_match(&ctx.mq)
            .await
            .map_err(|e| anyhow::anyhow!(e))?
            .map_err(|e| anyhow::anyhow!(e))
    }
    ...
    async fn update_station(
        ...
        updates: Vec<AllianceStationUpdate>,
    ) -> anyhow::Result<()> {
        token.auth(&ctx.kv)?.require_permission(&[
            Permission::FTA,
            Permission::FTAA,
    ...
    async fn estop_station(
        ...
        let mut stn = AllianceStation::get(&station_id, &ctx.kv)?;
        if astop {
            stn.set_astop(true, &ctx.kv)?;
        } else {
            stn.set_estop(true, &ctx.kv)?;
        }
        stn.insert(&ctx.kv)?;
        Ok(())
    }
