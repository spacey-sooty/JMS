use jms_core_lib::{
    db::{Singleton, Table},
    models::{
        CommittedMatchScores, Match, MatchType, MaybeToken, Permission, PlayoffMode, TeamRanking,
    },
    schedule::generators::{
        MatchGeneratorRPCClient, QualsMatchGeneratorParams, MATCH_GENERATOR_JOB_KEY,
    },
};
...
pub trait MatchesWebsocket {
    ...
    async fn next(&self, ctx: &WebsocketContext) -> anyhow::Result<Option<Match>> {
        Ok(Match::sorted(&ctx.kv)?
            .into_iter()
            .find(|m| m.match_type != MatchType::Test && !m.played))
    }
    ...
    async fn gen_quals(
        ...
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::ManageSchedule])?;
        MatchGeneratorRPCClient::start_qual_gen(&ctx.mq, params)
            .await?
    ...
    #[endpoint]
    async fn get_playoff_mode(
        &self,
        ctx: &WebsocketContext,
        _token: &MaybeToken,
    ) -> anyhow::Result<PlayoffMode> {
        Ok(PlayoffMode::get(&ctx.kv)?)
    }

    #[endpoint]
    async fn set_playoff_mode(
        &self,
        ctx: &WebsocketContext,
        token: &MaybeToken,
        mode: PlayoffMode,
        ...
        token
            .auth(&ctx.kv)?
            .require_permission(&[Permission::ManagePlayoffs])?;
        MatchGeneratorRPCClient::reset_playoffs(&ctx.mq)
            .await?
            .map_err(|e| anyhow::anyhow!(e))?;
        mode.update(&ctx.kv)?;
