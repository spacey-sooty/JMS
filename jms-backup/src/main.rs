...
use jms_backup_lib::{BackupSettings, JMSBackupRPC};
use jms_base::{
    kv::{self, KVConnection},
    logging::JMSLogger,
    mq::{self, MessageQueue},
};
use jms_core_lib::{
    db::{Singleton, Table},
    models::{EventDetails, JmsComponent},
};
...
impl JMSBackups {
    ...
    async fn do_backup(&self) -> anyhow::Result<()> {
        if let Some(event_code) = EventDetails::get(&self.kv)?.code {
            ...
            match self
                .backup_filesys(&settings, filename.clone(), &data)
                .await
            {
                Ok(()) => (),
