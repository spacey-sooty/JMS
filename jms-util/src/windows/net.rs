...
use windows::{
    ...
    Win32::{
        Foundation::{
            CHAR, ERROR_ACCESS_DENIED, ERROR_INVALID_PARAMETER, ERROR_NOT_SUPPORTED, ERROR_SUCCESS,
            WIN32_ERROR,
        },
        NetworkManagement::IpHelper::{
            AddIPAddress, DeleteIPAddress, GetAdaptersInfo, IP_ADAPTER_INFO,
        },
        Networking::WinSock::inet_addr,
    },
};
            ...
            while !i.is_null() {
                ...
                loop {
                    let network = Ipv4Network::with_netmask(
                        to_str(&ip.IpAddress.String).parse()?,
                        to_str(&ip.IpMask.String).parse()?,
                    )?;
...
pub async fn configure_addresses<I>(
    handle: &Handle,
    iface: &str,
    addresses: I,
) -> Result<(), anyhow::Error>
    ...
    I: IntoIterator<Item = Ipv4Network>,
{
    ...
    let iface = all_ifaces
        .iter()
        .find(|i| i.name == iface)
        .ok_or(anyhow::anyhow!("Link does not exist"))?;
        ...
        for addr in addresses {
            ...
            match result {
                ...
                ERROR_INVALID_PARAMETER => {
                    anyhow::bail!("Could not add address: Invalid Parameter")
                }
