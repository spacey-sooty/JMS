...
use quote::quote;
use syn::{
    parse_macro_input, spanned::Spanned, FnArg, GenericArgument, ItemTrait, Path, PathArguments,
    PathSegment, ReturnType, TraitItem,
};
                ...
                if let ReturnType::Type(_, typ) = return_type {
                    if let Some(return_type) = extract_type_from_result(typ) {
                        if f.attrs[0].path().is_ident("publish") {
                            ...
                            let last_published_ident =
                                syn::Ident::new(&format!("last_{}", f_ident), f_ident.span());
                            ...
                            rpc_response_schemas.push(quote! {
                {
                  ...
                  sub.push(schemars::schema::Schema::Object(s));
                            ...
                            return Err(syn::Error::new(
                                f.attrs[0].span(),
                                "Unrecognised Attribute, should be either `publish` or `endpoint`",
                            ));
                        }
                    } else {
                        return Err(syn::Error::new(
                            typ.span(),
                            "Endpoints and Publish functions should return an anyhow Result",
                        ));
                    }
                } else {
                    return Err(syn::Error::new(
                        f.span(),
                        "All endpoints and publish functions should return an anyhow::Result<>",
                    ));
                }
            }
        }
