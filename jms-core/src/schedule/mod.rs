...
use jms_core_lib::schedule::generators::{
    MatchGeneratorRPC, QualsMatchGeneratorParams, MATCH_GENERATOR_JOB_KEY,
};
use log::{error, info};

use self::{playoffs::PlayoffMatchGenerator, quals::QualsMatchGenerator};
...
impl MatchGeneratorRPC for GeneratorService {
    ...
    async fn start_qual_gen(&mut self, params: QualsMatchGeneratorParams) -> Result<(), String> {
        ...
        if kv
            .get::<bool>(MATCH_GENERATOR_JOB_KEY)
            .map_err(|e| e.to_string())?
        {
            return Err("Quals Generator is already running!".to_owned());
        }

        kv.set(MATCH_GENERATOR_JOB_KEY, true)
            .map_err(|e| e.to_string())?;
        kv.expire(MATCH_GENERATOR_JOB_KEY, 20 * 60)
            .map_err(|e| e.to_string())?; // 20 mins, just incase we panic
