...
use crate::{db::Singleton, models::Alliance};
...
impl LiveScore {
    ...
    pub fn derive(&self, other_alliance: &LiveScore) -> DerivedScore {
        ...
        let other_total_score =
            other_alliance.mode_score().total() as usize + self.penalty_points_other_alliance();
    ...
    pub fn update(&mut self, score_update: ScoreUpdate) {
        match score_update {
            ...
            ScoreUpdate::Penalty { fouls, tech_fouls } => {
                ...
                self.penalties.tech_fouls =
                    saturating_offset(self.penalties.tech_fouls, tech_fouls);
            }
        }
    }
    ...
    fn community_points(&self) -> ModeScore<isize> {
        ...
        for (row, cols) in self.community.auto.iter().enumerate() {
            ...
            ms.auto +=
                point_base * cols.iter().filter(|&x| *x != GamepieceType::None).count() as isize;
        }
    ...
    fn meets_coopertition(&self) -> bool {
        ...
        for els in self
            ...
            .auto
            .iter()
            .chain(self.community.teleop.iter())
        {
    ...
    pub fn randomise() -> Self {
        ...
        let rand_endgame = |rng: &mut ThreadRng| match rng.gen_range(0..=2) {
            0 => EndgameType::None,
