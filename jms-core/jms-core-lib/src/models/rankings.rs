...
use crate::{
    db::Table,
    scoring::scores::{DerivedScore, WinStatus},
};
...
impl TeamRanking {
    ...
    fn update_single(
        team: usize,
        score: &DerivedScore,
        current_rankings: &mut HashMap<usize, TeamRanking>,
    ) {
...
impl Ord for TeamRanking {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        cmp_f64(
            avg(self.rp as isize, n_self),
            avg(other.rp as isize, n_other),
        )
        .then(cmp_f64(
            avg(self.auto_points, n_self),
            avg(other.auto_points, n_other),
        ))
        .then(cmp_f64(
