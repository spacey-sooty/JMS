...
impl std::fmt::Display for AllianceParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AllianceParseError::InvalidStation(e) => {
                write!(f, "Could not parse station int: {}", e)
            }
        }
    }
}
...
impl AllianceStationId {
    ...
    pub fn to_id(&self) -> String {
        format!(
            "{}{}",
            self.alliance.to_string().to_lowercase(),
            self.station
        )
    }
    ...
    pub fn all() -> Vec<AllianceStationId> {
        vec![
            Self::new(Alliance::Blue, 1),
            Self::new(Alliance::Blue, 2),
            Self::new(Alliance::Blue, 3),
            Self::new(Alliance::Red, 1),
            Self::new(Alliance::Red, 2),
            Self::new(Alliance::Red, 3),
        ]
    }
}
...
impl FromStr for AllianceStationId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if &s[0..=2] == "red" && s.len() > 3 {
            let n: usize = s[3..]
                .parse()
                .map_err(|e| AllianceParseError::InvalidStation(e))?;
            Ok(AllianceStationId {
...
impl Match {
    ...
    pub fn has_team(&self, team: usize) -> bool {
        self.red_teams
            .iter()
            .chain(self.blue_teams.iter())
            .find(|&ot| (*ot) == Some(team))
            .is_some()
    }
