...
impl UnifiClient {
    ...
    pub async fn get(&self, fragment: &str) -> anyhow::Result<serde_json::Value> {
        let result = self.client.get(self.api_url(&fragment)).send().await?;
...
pub async fn configure_ap_teams(
    ...
    client
        .login(&settings.radio_username, &settings.radio_password)
        .await?;

    let networks = client
        .get_default("networkconf")
        .await?
        .get("data")
        .cloned()
        .ok_or(anyhow::anyhow!("No Networkconf Data Given"))?;
    let wlans = client
        .get_default("wlanconf")
        .await?
        .get("data")
        .cloned()
        .ok_or(anyhow::anyhow!("No WLAN Conf Data Given"))?;
        ...
        .clone()
    {
        ...
        client
            .put_default(&format!("wlanconf/{}", id), wlan)
            .await?;
    }
...
pub async fn force_reprovision(settings: &NetworkingSettings) -> anyhow::Result<()> {
    ...
    let devices = client
        .get("s/default/stat/device-basic")
        .await?
        .get("data")
        .cloned()
        .ok_or(anyhow::anyhow!("No Stat Data Given"))?;
