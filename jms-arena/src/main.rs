...
use jms_arena_lib::{
    AllianceStation, ArenaHookDB, ArenaRPC, ArenaSignal, ArenaState, HookReply, MatchPlayState,
    ARENA_STATE_KEY,
};
use jms_base::{
    kv::KVConnection,
    logging::JMSLogger,
    mq::{MessageQueue, MessageQueueChannel, MessageQueueSubscriber},
};
use jms_core_lib::{
    db::{Singleton, Table},
    models::{self, Alliance, AllianceStationId, JmsComponent, Match},
...
impl Arena {
    ...
    pub async fn commit_scores(&mut self, match_id: String) -> anyhow::Result<()> {
        ...
        if let Ok(mut m) = Match::get(&match_id, &self.kv) {
            ...
            m.red_teams = stns
                .iter()
                .filter(|x| x.id.alliance == Alliance::Red)
                .map(|x| x.team)
                .collect();
            m.blue_teams = stns
                .iter()
                .filter(|x| x.id.alliance == Alliance::Blue)
                .map(|x| x.team)
                .collect();
        ...
        match self.state.clone() {
                ...
                if signal == Some(ArenaSignal::Prestart) {
                    match &self.current_match {
                        Some(m) if m.state == MatchPlayState::Waiting => {
                            self.set_state(ArenaState::Prestart { ready: false })
                                .await?;
                        }
                        Some(m) => {
                            anyhow::bail!("Cannot Prestart when Match is in state: {:?}", m.state)
                        }
                        None => anyhow::bail!("Cannot prestart without a match loaded!"),
                    }
                }
            }
            ArenaState::Prestart { ready: false } => match self.ready().await {
            ...
            ArenaState::MatchPlay => {
                let current_match = self
                    .current_match
                    .as_mut()
                    .ok_or(anyhow::anyhow!("No match present!"))?;
                if first {
                    current_match.start()?;
                }
                ...
                match current_match.state {
                    MatchPlayState::Complete => {
                        self.set_state(ArenaState::MatchComplete).await?;
                    }
                    _ => (),
                }
            }
        ...
        match self.state {
            ArenaState::Idle { .. } => {
                ...
                for (i, team) in m.blue_teams.into_iter().enumerate() {
                    ...
                    self.stations
                        .get_mut(&id)
                        .ok_or("No Station Available".to_string())?
                        .set_team(team, &self.kv)
...
impl Arena {
    async fn run(&mut self) -> anyhow::Result<()> {
        ...
        let mut hook_replies: MessageQueueSubscriber<HookReply> = self
            .mq
            .subscribe("arena.state.hook", "arena-hooks", "arena", false)
            .await?;
