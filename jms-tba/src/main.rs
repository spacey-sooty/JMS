...
use crate::{
    alliances::TBAAlliances, eventinfo::TBAEventInfoUpdate, matches::TBAMatchUpdate,
    rankings::TBARankings, teams::TBATeams,
};
...
async fn do_update(kv: &kv::KVConnection) -> anyhow::Result<()> {
    ...
    // Event Info
    if let Err(e) = TBAEventInfoUpdate::issue(&kv).await {
        warn!("Could not issue event info update: {}", e)
    }

    // Teams
    if let Err(e) = TBATeams::issue(&kv).await {
        warn!("Could not issue teams update: {}", e)
    }

    // Alliances
    let alliances: TBAAlliances = models::PlayoffAlliance::all(&kv)?.into();
    if let Err(e) = alliances.issue(&kv).await {
        warn!("Could not issue alliance update: {}", e)
    }

    // Rankings
    let rankings: TBARankings = models::TeamRanking::all(&kv)?.into();
    if let Err(e) = rankings.issue(&kv).await {
        warn!("Could not issue ranking update: {}", e)
    }

    // Matches
    if let Err(e) = TBAMatchUpdate::issue(&kv).await {
        warn!("Could not issue teams update: {}", e)
    }
