...
use jms_base::{
    kv,
    logging::JMSLogger,
    mq::{self, MessageQueueChannel},
};
use jms_core_lib::{
    db::Table,
    models::{Alliance, JmsComponent},
};
use log::{info, warn};
use tokio::{
    ...
    loop {
        ...
        .await
        {
            Ok(result) => {
                ...
                if char == 'E' {
                    match ArenaRPCClient::signal(
                        &mq,
                        jms_arena_lib::ArenaSignal::Estop,
                        "Field Electronics (Scoring Table)".to_owned(),
                    )
                    .await?
                    {
                        Ok(()) => (),
...
async fn main() -> anyhow::Result<()> {
    ...
    let ds_elec_futs = if std::env::var("DS_ELECTRONICS").is_ok() {
        let blue = DriverStationElectronics::new(Alliance::Blue, None, kv.clone()?);
        let red = DriverStationElectronics::new(Alliance::Red, None, kv.clone()?);
