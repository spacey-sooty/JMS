...
use jms_arena_lib::{AllianceStation, ArenaState, ArenaStateHook};
use jms_base::{kv, logging::JMSLogger, mq};
use jms_core_lib::{
    db::{Singleton, Table},
    models::{self, Alliance, AllianceStationId, JmsComponent},
};
use jms_networking_lib::{JMSNetworkingRPC, NetworkingSettings, RadioType};
use log::{error, info};
use tokio::try_join;
...
async fn do_team_network_update(
    network: NetworkConfig,
    settings: NetworkingSettings,
) -> anyhow::Result<()> {
    info!("Starting Network Update...");
    pfsense::configure_firewall(&network, &settings)
        .await
        .map_err(|e| anyhow::anyhow!("PFSense Error: {}", e))?;
    match settings.radio_type {
        RadioType::Linksys => linksys_ap::configure_ap_teams(&network, &settings)
            .await
            .map_err(|e| anyhow::anyhow!("AP Error: {}", e))?,
        RadioType::Unifi => unifi::configure_ap_teams(&network, &settings)
            .await
            .map_err(|e| anyhow::anyhow!("AP Error: {}", e))?,
    }
...
impl JMSNetworkingRPC for NetworkingService {
    ...
    async fn configure_admin(&mut self) -> Result<(), String> {
        ...
        if settings.radio_type == RadioType::Linksys {
            linksys_ap::configure_ap_admin(&settings)
                .await
    ...
    async fn force_reprovision(&mut self) -> Result<(), String> {
        ...
        if settings.radio_type == RadioType::Unifi {
            unifi::force_reprovision(&settings)
                .await
                .map_err(|e| e.to_string())
        } else {
            return Err("Not Supported".to_owned());
...
impl NetworkingService {
    pub async fn run(
        &mut self,
        mut hook_reset: ArenaStateHook,
        mut hook_prestart: ArenaStateHook,
    ) -> anyhow::Result<()> {
...
async fn main() -> anyhow::Result<()> {
    ...
    let hook_reset = ArenaStateHook::new(
        "jms.networking.reset",
        &component,
    ...
    .await?;
    let hook_prestart = ArenaStateHook::new(
        "jms.networking.prestart",
        &component,
    ...
    let mut networking = NetworkingService {
        kv: kv.clone()?,
        mq: mq.channel().await?,
    };
    ...
    try_join!(
        component_svc,
        networking.run(hook_reset, hook_prestart),
        imaging.run(kv)
    )?;
